...
- 開始頁、資料夾裡和「已收藏」都是一份目錄：一行一篇，左邊一枚小縮圖（md 是一張紙，紙上畫着這篇的骨架——哪裡是小標題，哪裡是段落、列表、表格；html 是一扇小窗），中間是標題，一串點把眼睛引到最右邊的時間，底下一行灰字是內文的第一句。還是可以按「全部 / Markdown / HTML」篩選。
...
- 開始頁有「最近打開」，還有一張「上次停在」的大卡——上次讀到哪篇，一眼看見，按「接着讀」就回去。列表上的時間是你上次打開它的時刻，大卡那一篇不會在下面再列一次。
- 開始頁平時不顯示狀態；只有正在整理（「掃描中…」）或服務未啟動時，問候語旁邊才出一枚小點。視窗底下也不掛狀態列。
- 在筆記上按右鍵選「收藏」（文件標題列右邊那顆星一樣得），收起來的都在「已收藏」那枚按鈕裡。
...
- 內文裡寫的 `- [ ]` 係真係撳得嘅勾選框：讀住就勾得，勾完文字變灰劃一道線，嗰一行的 `[ ]` 即刻變成 `[x]` 寫返入檔案——只改方括號裡那一個字。編輯態裡按 Return，接住起下一條。
...
- 在 Finder 裡按兩下 `.md`，就用它打開。會開在一個新分頁裡，正在讀的那篇不受影響。

**幾個筆記本**

- 一個資料夾就是一個**筆記本**。加了第二個之後，開始頁問候語下面多一行筆記本按鈕：`全部 | ● 工作 | ● 讀書 | ☆ 已收藏 | ＋`，資料夾按鈕、列表、搜尋範圍和新增落點都跟着它走。
- 每本一種顏色。列表、路徑按鈕、分頁和 ⌘K 結果上都帶這枚色點，一眼知道在哪一本裡；紫色仍舊只表示「選中」。
...
`X-AMN-Agent` 所有 POST 都認：流水上記成「来源 Agent · 代理 你的名字」，列表裡那一行的標題後面也會多一枚 `✦ 你的名字`。

改檔案內容的只有 `/__save` 和 `/__task` 兩條（後者只把某一行 `- [ ]` 方括號裡那一個字翻過來），只寫庫根以內的 `.md`，寫之前先留一版備份。另外兩條搬位置的是 `/__trash` 和 `/__untrash`：檔案原樣進出系統垃圾桶，一個位元組都不會動。備份資料夾、快取資料夾、隱藏資料夾一律不給動。
...
**Agent 改了我的筆記我怎麼知道？** 列表裡那一行會多一枚 `✦ Claude Code`——這一篇最近一次是本機哪個 AI 助手寫的，七天內都標着。要看細賬翻 `.amnote/changes.jsonl`，每條都記着「来源」和「代理」。

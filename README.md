...
<p align="center">把本机一个文件夹当成笔记库来用。<br>窗口像浏览器：地址栏搜这个文件夹，书签栏是顶层子文件夹。<br>文件留在原地，不会搬家，也不会上传到网上。</p>
...
已经有一堆 Markdown、不想再搬进云笔记的时候，把它指到那个文件夹就行。md 和 html 都在 App 里打开。搜一搜、改一改，还是原来那些文件。

![首页：搜索整个库](docs/home.png)

## 能做什么

- **从地址栏搜。** 搜的是所选文件夹，不是网上。⌘K 按名字直达。
- **在 App 里看 md 和 html。** 点开就在标签里读；双击进编辑，停笔两秒自动保存。
- **书签栏是文件夹。** 库的顶层子文件夹排在上面，不需要可以藏起来。
- **⌘T 开一个新标签。** 先到开始页：搜一下，点最近的文件，或再新建随手记。⌘N 再开一个窗口。

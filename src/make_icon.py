...
菜单栏那两张 template（menubar-cloud.png / @2x）**这个脚本只读不写**。
5.4 换成珍珠紫玻璃母版之后，底板本身就是淡紫的 (243,227,252)，
旧的「按亮度阈值从中心洪水填充抠白云」会把整块底板当成云，
抠出来是一坨方斑。菜单栏那颗云改成手工维护的独立模板，跟母版脱钩。
"""
...
# 手工维护，不从母版生成（见文件头）。这里只用来核对文件还在不在。
MENUBAR = [
...
def keep_menubar():
    """菜单栏 template 是手工资源，只核对存在，不重新生成。

    返回还在的那几个路径；缺文件时返回空列表（壳里会退到 SF cloud.fill）。
    """
    kept = []
    for name, _size in MENUBAR:
...
def main(argv):
    ...
    for name, size in PWA:
        ...
        if size in cache:
            ...
            cache[size] = p
        note(p)
    print('菜单栏小云：沿用现成模板，不从母版抠')
    for p in keep_menubar():
        print('  %-40s %8.1f KB（保持不变）' % (os.path.basename(p),

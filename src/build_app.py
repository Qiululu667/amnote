...
# 4.4.5 = 随手记列表记号与回车光标（20260831，Grok 4.6）：空段选区按块记、
# `- ` / `1. ` 转列表、空列表项回车退出。壳没改，版本号跟着门户走，方便关于里对得上。
# 4.4.4 = 工具栏真正收小（Small 模式 + AMNoteMain5）并修关掉侧边栏后首页叠层（20260830，Grok 4.6）。
# 4.4.3 = 工具栏圆角图标从 23pt 收到 18pt（20260830，Grok 4.6）。
# 4.4.2 = 首页纯白、最近文件不再叠白卡片、工具栏圆角图标（20260830，Grok 4.6）。
# 4.4.1 = 访达双击 md 真正打开那份（20260830）：冷启动时 openURLs 早于
# didFinishLaunching，队列必须在 init 里建好，网页没就绪不许倒掉。
# 版本号是唯一能在「关于 AM·Note」里看出来跑的是新壳还是旧壳的地方，改了壳就要动它。
NATIVE_VERSION = ("5.0.0", "20")
MIN_MACOS = "12.0"

...
# 5.13.0 = 顶栏去掉前进后退（⌘[ / ⌘] 仍走键盘和菜单）；头像从右上角并进开始页问候；
# 标签条从冷灰改成跟云朵 icon 同色温的珍珠紫玻璃洗（--chrome 浅 #e6daf0 / 深 #2a2734，
# chromeColorForTheme 同步，不然红绿灯背后会露出旧灰）；搜索胶囊在条里上移 3px。
...
# ——服务端的「骨架」照算照带，只是前端不再消费。壳本身一行没改，只跟版本号。
# 5.10.0 = 开始页 / 收藏页 / 文件夹页三处卡片网格换成目录式列表（一行一篇：缩略图｜标题
# ···· 时间｜一行摘要），缩略图接着用 5.9 的「骨架」画，卡片那一套整段退役；
# 「最近打开」的时间列改写「打开于」（ST.recents 每次打开盖一个时间戳）；
# list_preview 洗法按「第一段散文」重写（跳过标题 / 代码 / 引用 / 表格 / 链接行 /
# 纯日期行，清单类退到「项目名串一句」那一档）——CLI / MCP 看到的「预览」跟着变。
# Markdown 里的 `- [ ]` / `- [x]` 在阅读态可以直接点，只把那一行方括号里的一个字节
# 翻过来（新路由 POST /__task，带 mtime + 当前行两道冲突闸）；编辑态回车续一条待办。
# 壳本身一行没改，只跟版本号。
